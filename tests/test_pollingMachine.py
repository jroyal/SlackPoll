...
class TestPollingMachine(TestCase):
    def test_create_poll_no_other_poll(self):
        pm = PollingMachine("http://abc.com")
        poll = pm.create_poll("James", "#jamestestchannel", "Test topic", [{"name": "option1", "count": 0},
    ...
    def test_create_poll_existing_different_channel_poll(self):
        pm = PollingMachine("http://abc.com")
        pm.create_poll("James", "#jamestestchannel", "Test topic", [{"name": "option1", "count": 0},
                                                                    {"name": "option2", "count": 0}])
        poll = pm.create_poll("James", "#jamestestchannel2", "Test topic", [{"name": "option1", "count": 0},
                                                                            {"name": "option2", "count": 0}])
        assert isinstance(poll, Poll)
